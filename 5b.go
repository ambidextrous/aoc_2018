...
//func getShortestPolymerMultithread(unalteredPolymer string) int {
//	alphabetLower := "abcdefghijklmnopqrstuvwxyz"
//	alphabetUpper := "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
//	shortest := len(unalteredPolymer)
//	for i:=0; i<len(alphabetLower); i++ {
//		jobs := make(chan int, len(alphabetLower))
//		newPolymer := strings.Replace(unalteredPolymer, string(alphabetLower[i]), "", -1)
//		newPolymer = strings.Replace(newPolymer, string(alphabetUpper[i]), "", -1)
//		go getReducePolymerLength(newPolymer, rune(alphabetLower[i]), jobs)
//	}
//	totals = [len(alphabetLower)]int{}
//	for i:=0; i<len(alphabetLower); i++ {
//		tot := <- jobs
//		jobs = append(jobs, tot)
//	}
//	return shortest
//}
